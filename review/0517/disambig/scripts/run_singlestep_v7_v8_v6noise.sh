...
LOG=${LOG_DIR}/singlestep_v7_v8_v6noise_gpu${GPU:-1}_${STAMP}.log
run_eval() {
  local exp="$1"
  local kind="$2"
  ...
  CUDA_VISIBLE_DEVICES=${GPU:-1} /home/qujiaxiang/.conda/envs/rae/bin/python -u tools/eval_per_hop_singlestep_clip3.py \
    --config "$cfg" \
    --checkpoint "$ckpt" \
    --tag "$tag" \
