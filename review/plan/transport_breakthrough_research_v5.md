# Transport 突破实验计划 v5 — 2026-04-26（**自顶向下重构版**）

**Supersedes**: v4（SF-pair 方向已暂停）
**策略转换**：~~先分析原理 → 再实验~~ → **先冲一个能看的效果 → 再回头解释**
**核心方向**: Rollout-Heavy（加大已有 rollout_loss 权重）
...
```
Step 1（今天）：直接启动 V5-1B 激进版（rollout λ=2.0, image_aux=0.04, step_weights=[0.5, 1.0, 2.0, 4.0]）
Step 2（+5K~+10K 步）：看 val_chain_normal_mse 是否改善
Step 3（如果改善）：跑 full-val + Path A，得到一个"看得过去"的结果
Step 4（事后）：回头分析为什么有效 / 为什么这种配置突破了瓶颈
Step 5（可选）：B4/B5/方向 5 这些机制验证作为论文写作时的补强证据，不阻塞主实验
```

**关键变化 vs 旧 V5**：
- ~~先做 1A 保守版 → 验证趋势 → 再升 1B~~
- ✅ 直接 1B：因为 1A 的 4× rollout 在已知"梯度占比从 10%→30%"的情况下，**改善幅度可能仍在 baseline 噪声内（参考 V4 SF best 也只有 5%）**，不足以判断方向是否成立
- 1B 的 8× rollout 直接把 roll_frac 推到 40-60%，**如果方向对**，应当看到肉眼可分辨的改善（≥10%）；如果仍持平，**说明方向错了**——这才是有信息量的实验
- 为防相变，保留 §2.1 val_pair_total 监控（成本 0）
...
> 完整的 SF vs rollout 机制对比表移到 §10（事后分析）。这里只列**直接启 1B 的最大风险**和**为什么可接受**。

**最大风险**：1B 的 8× rollout + image_aux ÷3 同时大幅改动，可能像 V4 SF 一样触发参数盆地切换（val_pair_total 跳变）。

**为什么可接受**：
1. **rollout 在 V3 200K 已经存在 λ=0.25 并稳定**——加大它是放大已被内化的信号，**不引入新分布**
2. **每步 GT 监督**——rollout 每跳都有 `||z_pred - z_gt||²` 拉力，不让参数飘
3. **可即时回退**：如果 +5K val_pair_total > 0.00005，立即停 1B 跳到 1A 保守版
4. **机会成本低**：50K 步 ~30h，相比"先 1A 50K + 再 1B 50K"的 60h，1B 直接上反而省时间

**接受这个赌注的判据**：我们现在不知道方向是否对；保守 1A 即使成功也只能给出"小幅改善"——不足以判断是否值得继续。激进 1B 给出**明确信号**：要么明显改善（押对了）要么明显失败（押错了，少浪费时间）。
