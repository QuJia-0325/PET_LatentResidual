...
echo "  rollout lambda:    $(grep 'lambda_start: 1.50' ${CONFIG} | head -1 || echo 'NOT FOUND ⚠️')"
echo "  image_aux lambda:  $(grep 'lambda_start: 0.08' ${CONFIG} | head -1 || echo 'NOT FOUND ⚠️')"
echo "  lr:                $(grep 'lr: 8.0e-5' ${CONFIG} | head -1 || echo 'NOT FOUND ⚠️')"
echo "  ema enabled:       $(grep -A1 '^ema:' ${CONFIG} | grep 'enabled: true' || echo 'NOT FOUND ⚠️')"
echo "  SF disabled:       $(grep 'enabled: false' ${CONFIG} | head -1 || echo 'NOT FOUND ⚠️')"
...
echo "  +10K 步: grep 'val_chain_normal_mse' → 应有下降趋势（注: baseline val 需从第一次 [val] 行读取，不是 best_val）"
echo "  +25K 步: val_chain_normal_mse 应持续改善"
echo "  +50K 步: full-val eval + Path A redo"
echo ""
echo "  注: best_val=${RESUME_VAL} 是 selection score（val_multi_objective），不是 val_chain_normal_mse"
echo ""
echo "=== 止损条件 ==="
