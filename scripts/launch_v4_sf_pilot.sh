...
echo "       resume: ${V3_BEST}"
echo ""
echo "--- 预期 SF 调度 (50K run) ---"
echo "  step ${RESUME_STEP} ~ $((RESUME_STEP + 5000)):  sf_alpha=0 (warmup, 纯 GT)"
echo "  step $((RESUME_STEP + 5000)) ~ $((RESUME_STEP + 15000)): sf_alpha 0→1 (ramp)"
echo "  step $((RESUME_STEP + 15000)) ~ ${MAX_STEPS}:  sf_alpha=1 (稳态 SF, 35K 步)"
echo ""
...
echo ""
echo "=== 检查点 ==="
echo "  +5K步:  sf_alpha 应仍为 0.000 (warmup 区间)"
echo "  +10K步: sf_alpha 应 ~0.5 (ramp 中)"
echo "  +15K步: sf_alpha=1.000, sf_gap 应 > 0"
echo "  +30K步: val 趋势应可见（若仍退化 >10% 考虑止损）"
echo "  +50K步: 完成，跑 full-val eval"
